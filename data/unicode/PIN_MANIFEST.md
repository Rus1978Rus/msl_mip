...
ДОМЕННЫЕ РЕЕСТРЫ (2026-08-07, вне UCD-трека — отдельная директория data/net/):
| Файл | Источник | sha256 | Снято |
|---|---|---|---|
| net/tlds-alpha-by-domain.txt | IANA root zone (1438 TLD, версия 2026080600) | 932b5f31c5ef487ff7c4e70ef7d159a2ddc41e5d827d60a5fce9320d806c7b6d | 2026-08-07 |
| net/public_suffix_list.dat | publicsuffix.org (8509 составных суффиксов) | 084a5674d77c1d14900b16da5fc8afee9765af2f00a638552a8c7aa18f44ae81 | 2026-08-07 |
Загрузчик core/public_suffix.py. **ВЕНДОРЕННЫЙ СНАПШОТ — ИСТОЧНИК ПО УМОЛЧАНИЮ**; сеть
включается ТОЛЬКО переменной `MSL_MIP_ALLOW_NETWORK=1` (тогда порядок прежний: live -> кэш ->
вендор -> встроенный огрызок). Основание: air-gapped установка молча работала на встроенных
163 TLD вместо 1438 И сообщала о себе «не деградировано», потому что проверка смотрела на ЧИСЛО
записей, а строку источника выбрасывала. Деградация теперь определяется ПРОИСХОЖДЕНИЕМ
(`is_full_registry`), порог размера сохранён как независимое второе условие. Несовпадение sha256
или отсутствие файла → снапшот НЕ используется (падаем на следующий уровень, источник виден).
Дельта-перепись при переходе огрызок→вендор: 0 сдвинутых вердиктов на корпусе из 19 доменов.
Гейт: tests/gate_vendored_registry.py. Эти реестры ЖИВЫЕ (меняются еженедельно) — их бамп
не требует отдельного AUTHOR_DECISION, но требует пересчёта sha256 и прогона гейта.
