...
DOCUMENT_ID: SIGN_CORE_CARD_TEMPLATE_GEN3_v0_3_R1_RU
...
CONFUSABLES:  [МИНИМУМ 5 — ДОКУМЕНТАЦИЯ/ПРОВЕНАНС, НЕ рантайм]
  [РЕВИЗИЯ R1 (ось «отношение», AUTHOR_DECISION_20260708): CONFUSABLES —
   человекочитаемый список похожих знаков для людей и истории. РАНТАЙМ
   этот блок КАК РЁБРА НЕ читает. Активные отношения знака (что рантайм
   реально использует) живут НИЖЕ, в блоке SIGN_RELATIONS. Одна связь,
   если нужна рантайму, описывается в SIGN_RELATIONS, НЕ дублируется тут.]
  CONFUSABLE_001:
...
SIGN_RELATIONS:  [ОПЦИОНАЛЬНО — блок отношений знака; ИСТОЧНИК ИСТИНЫ
  для РАНТАЙМА (ось «отношение», AUTHOR_DECISION_20260708, D-REL-2).
  ОТСУТСТВИЕ блока = у знака нет активных отношений (legacy/самостоятельный
  знак, D1). Рёбра заводятся ТОЛЬКО ЯВНО, с указанным CONTEXT_SCOPE.]
  RELATION_001:
    RELATION_TYPE: <CONFUSABLE_OF / NFKC_MAPS_TO / VISUAL_MIMIC_OF>
    TARGET: <кодпоинт или последовательность канона, напр. U+002F>
    CONTEXT_SCOPE: <один или несколько через запятую из:
      URL / HOST / PORT / PATH / EMAIL / IDENTIFIER / IDN / CODE /
      FREE_TEXT / ANY>
      [HOST = доменная часть (атака gοοgle.com ловится тут);
       ANY = «маска везде», применять ОСТОРОЖНО — высокий риск ложных
       срабатываний, только для контекст-независимых связей]
    VERIFICATION_STATUS: <VERIFIED / CANDIDATE / MANUAL_OVERRIDE>
    RUNTIME_EFFECT: RELATION_ONLY
      [ЗАШИТЫЙ ИНВАРИАНТ: ребро сообщает «похож на TARGET в CONTEXT_SCOPE»
       и НИЧЕГО про риск. RELATION_FOUND ≠ THREAT. Риск маски выносит
       SEQUENCE-слой (ребро + защищённый контекст + соседи), D-REL-4;
       single-sign для маски эмитит risk=NONE + кандидата отношения.]
  (повторять RELATION_002, ... по числу отношений; может быть 0)
