...
CONFUSABLES:
  ...
  FUNCTIONAL_NEIGHBORS:
    [ТЕРМИН ОБНОВЛЁН 2026-07-16: пять знаков ниже — это ОБРАЗЦЫ (представители
     ПОДНАДЗОРНОГО КЛАССА невидимых), НЕ «все соседи». «Сосед/NEIGHBOR» — LEGACY
     (глоссарий: foundation_layer/CLASS_FRONT_INVISIBLE_SIGNS.md). Поднадзорный
     класс = Cf∧Default_Ignorable = 138 (D-NEIGHBORS-BORDER-138); эти 5 ⊂ 138.
     Имя поля FUNCTIONAL_NEIGHBORS и ключи NEIGHBOR_00N СОХРАНЕНЫ как СТРУКТУРНЫЕ
     (шаблон/preflight) — переименование поля отдельным заходом.
     НЕ спутываемость ПО ВИДУ (не CONFUSABLES, различие ФУНКЦИИ, не вида);
     справочный блок для человека; рантайм риск по ним НЕ считает — ключей
     CONFUSABLE_ здесь нет, парсер их не грузит, и это НАМЕРЕННО.]
    NEIGHBOR_001:
      CODEPOINT: U+200C
