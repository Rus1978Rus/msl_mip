...
RISK_CASES:
  ...
  RISK_CASE_003:
    ...
    THREE_LEVEL: ВОЗМОЖНАЯ (закрыто замером — AUTHOR_DECISION_20260721_D-O1-TOKEN-CELL)
    ...
    NOTE: РАНЕЕ здесь стояло «ЖЕЛАЕМО РЕАЛЬНАЯ». Замер FP-бюджета (2026-07-21) это ожидание
      ЗАКРЫЛ: прямое правило BOM×BYTE_EXACT_TOKEN→hold эскалирует 67% БЕЗОБИДНЫХ текстов с BOM
      (CSV/Excel-экспорт, склейка файлов, вставка из дока, лог, markdown, проза). При такой
      базовой ставке hold перестаёт быть сигналом. Определение шкалы — «часто легитимен, но НЕ
      освобождается» — есть ТОЧНОЕ описание этой ячейки; значит честный уровень и есть ВОЗМОЖНАЯ.
      Дивергенция была «желанием без base-rate», а не ошибкой рантайма. Silent pass отсутствует:
      ghost-токен очередится (queue), человек его видит.
    ЧТО ДОСТАВЛЕНО ВМЕСТО ЭСКАЛАЦИИ: каноническая ПРОЕКЦИЯ (P1) — отчёт показывает collapse-форму
      и позиции вставки («выглядит как paypal.com; фактически pay<U+FEFF>pal.com»). Показ сути,
      не подъём громкости. Проекция — ТОЛЬКО показ, НИКОГДА не нормализация-для-решения.
    ПУТЬ К РЕАЛЬНАЯ (инкремент-2, измерен): предикат ФОРМЫ КОЛЛАПСА — collapse токена ∈ allowlist
      ПОТРЕБИТЕЛЯ (защищённые цели). Замер на одних корпусах: benign FP 0%, ghost TP 100% (против
      36%/90% у позиционного предиката и 67%/100% у прямого правила). Политико-связан: без списка
      потребителя инертен, отсутствие списка ≠ «чисто». Требует своего author decision.
  RISK_CASE_004:
...
THREE_LEVEL_DIVERGENCE (желаемое автором ↔ измеренное рантаймом — честно;
  ...
  РАСХОЖДЕНИЯ: ОСТАЛОСЬ 1 (было 4) — ведущий-перед-доменом [OPEN → инкремент-2, нужны
    ingress-метаданные: на offset 0 BOM может быть легит транспортным stream-start].
  ЗАКРЫТО ЗАМЕРОМ (3) — ПРАВКОЙ ОЖИДАНИЯ, не кода:
    токен + идентификатор → ВОЗМОЖНАЯ окончательно (AUTHOR_DECISION_20260721_D-O1-TOKEN-CELL).
      Замер FP-бюджета: прямое правило подняло бы 67% БЕЗОБИДНЫХ текстов с BOM в hold. При
      такой базовой ставке hold перестаёт быть сигналом. «Часто легитимен, но не освобождается»
      = определение ВОЗМОЖНАЯ ⇒ измеренное и есть честное. Вместо эскалации доставлена
      каноническая ПРОЕКЦИЯ (показ collapse-формы). Путь к РЕАЛЬНАЯ измерен: предикат формы
      коллапса (collapse ∈ allowlist потребителя; benign FP 0%, ghost TP 100%) — инкремент-2.
    email → ВОЗМОЖНАЯ окончательно (AUTHOR_DECISION_20260721_D-O1-IMPL-SCOPE). Замер: 6/6
      правдоподобно-БЕЗОБИДНЫХ email-входов (CSV-экспорт, вставка из дока, блок подписи)
      измеряются как EMAIL/MEDIUM ⇒ эскалация затопила бы hold обыденными случаями.
  СНЯТО РАНЕЕ (1): «ведущий BOM перед JSON — желаемо ВОЗМОЖНАЯ, измерено ЧИСТО» — снято
  ...
  ПРИЧИНА (уточнена замером): рантайм присваивает уровень по КОНТЕКСТУ (только HOST→hold;
    BYTE_EXACT_TOKEN/EMAIL/HIDDEN_BOUNDARY_PADDING→queue; FREE_TEXT→pass). РАНЕЕ расхождение
    объяснялось «рантайм не умеет поднять» и упиралось в риск контаминации ZWSP. Замеры
    2026-07-21 показали более глубокую причину: в этих контекстах БЕЗОБИДНОЕ ЧАСТО — то есть
    желаемая РЕАЛЬНАЯ была ЖЕЛАНИЕМ БЕЗ БАЗОВОЙ СТАВКИ, а не блокированной правдой. Дивергенция
    закрывается не подъёмом рантайма, а исправлением ожидания (и показом сути вместо громкости).
  СТАТУС: 3 из 4 ЗАКРЫТЫ author decision'ами по замеру; 1 (ведущий-перед-доменом) OPEN.
    Silent pass отсутствует нигде: все эти ячейки очередятся (queue), человек их видит.
