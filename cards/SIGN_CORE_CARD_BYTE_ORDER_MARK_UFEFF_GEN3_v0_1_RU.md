...
SAFE_CASES:
  ...
  SAFE_CASE_002:
    ...
    THREE_LEVEL: измерено ЧИСТО; ЖЕЛАЕМО ВОЗМОЖНАЯ (см. THREE_LEVEL_DIVERGENCE)
    EXPECTED: измерено INFO/pass
    RISK: измерено NONE
    RUNTIME_VERIFIED: YES [проба: "﻿{...}" → FREE_TEXT → pass. РАСХОЖДЕНИЕ с дизайном:
      автор хотел ВОЗМОЖНАЯ (queue), рантайм даёт ЧИСТО (pass) — FREE_TEXT риска не
      эмитит. Поднять нельзя без изменения рантайм-риск-мэппинга (задевает всё). OPEN.]
  SAFE_CASE_003:
...
THREE_LEVEL_DIVERGENCE (желаемое автором ↔ измеренное рантаймом — честно, 5 расхождений):
  Совпало (5): mid-host→РЕАЛЬНАЯ; host/tld→РЕАЛЬНАЯ; free-text→ЧИСТО;
    ведущий-текст→ВОЗМОЖНАЯ; хвостовой→ВОЗМОЖНАЯ.
  РАСХОЖДЕНИЯ (5) — ЖЕЛАЕМО РЕАЛЬНАЯ, ИЗМЕРЕНО ВОЗМОЖНАЯ (queue, не hold):
    токен, идентификатор, email, ведущий-перед-доменом.
  РАСХОЖДЕНИЕ — ЖЕЛАЕМО ВОЗМОЖНАЯ, ИЗМЕРЕНО ЧИСТО (pass): ведущий BOM перед JSON.
...
OQ2:
  QUESTION: THREE_LEVEL_DIVERGENCE — 5 контекстов, где желаемый уровень (РЕАЛЬНАЯ/
    ВОЗМОЖНАЯ) не совпал с измеренным. Эскалация требует рантайм-риск-мэппинга.
  STATUS: OPEN (привязано к застолблённому принципу трёх уровней + риск-градации 138)
  BLOCKS_WORKINGLY_CLOSED: NO
...
LIMITATION_STATEMENT:
  ...
  PROBE ≠ FULL_BATTERY (двуногой BY_SPEC+reconcile+mutation-adequacy для BOM ещё нет)
  THREE_LEVEL_IS_FIRST_APPLICATION: принцип трёх уровней применён здесь ВПЕРВЫЕ, на
    ...
    документ, риск-градация 138, возможный рантайм-уровень) — ОТДЕЛЬНЫЙ заход. 5
    расхождений желаемое↔измеренное честно вынесены в THREE_LEVEL_DIVERGENCE / OQ2.
