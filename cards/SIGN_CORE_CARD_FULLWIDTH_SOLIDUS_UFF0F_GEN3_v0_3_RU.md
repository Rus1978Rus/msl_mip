...
REVISION_NOTE_v0_5: 2026-07-12 — FIX_FIRST по второму КРУГУ (детектор в
  сборе). Адверсариальный ревьюер (1 из 6) нашёл два реальных обхода,
  воспроизведены трассировкой:
  БЛОКЕР 1: D-DET-4 сам создал обход — ведущий структурный символ +
    домен-С-маской (#goog<mask>le.com) глушился в FREE_TEXT. Починено:
    ведущий структурный префикс отделён от домена; демотится только
    маска ПОСЛЕ домена (#example.com<mask>x остаётся FREE_TEXT), домен
    С маской после префикса → HOST.
  БЛОКЕР 2: полноширинные/иероглифические точки (U+FF0E ., U+3002 。,
    U+FF61 ｡) канонизируются в ASCII-точку перед разбором меток —
    объект защиты сам fullwidth (／), атакующий поставит и fullwidth
    точку. gоog<mask>le<fullwidth-dot>com → HOST.
  Сторож (msl_mip_runtime): D-GUARD-3 — сравнение СЕРЬЁЗНОСТИ (HIGH,
    занижённый до log_only/queue, ловится, не только буквальный PASS;
    форма risk_level нормализуется); D-GUARD-4 — три поля вердикта
    (semantic/integrity_status/effective), эффективный поднимается до
    безопасного при нарушении, в окне видны оба. gate 40→55, chaos-gate
    15→23. Отдельные файлы AUTHOR_DECISION D-GUARD-3/4.
AUTHOR_DECISION_REFERENCE: foundation_layer/AUTHOR_DECISION_20260712_MASK_PATH_SAFEGUARD_D-GUARD-3.md, D-GUARD-4.md (+ ROUND2/ROUND1, + D-DET-1_2, + D-GUARD-1/2)
...
RISK_CASES:
  ...
  RISK_CASE_005:
    ...
    RULE: покрыт детектором голого домена Г1 (реконструкция
      ...
      TLD-справочник → тревога + прогон DEGRADED (D-DET-2). Ведущий
      структурный символ + домен-с-маской (#goog<mask>le.com) → HOST
      (Blocker 1, круг 2): префикс не глушит домен. Полноширинные/
      иероглифические точки (U+FF0E/U+3002/U+FF61) канонизируются в
      ASCII перед разбором (Blocker 2, круг 2). Код детектора —
      NEW_ARTIFACT, NOT_REVIEWED; карточка и детектор идут на конвейер
      одним пакетом
...
- ИСПРАВЛЕНО с 2026-07-12 (FIX_FIRST, узкий круг, D-DET-4; уточнено в
  круге 2 → Blocker 1): ведущий структурный разделитель (/ \ ? # : @)
  перед доменом — НЕ часть голого домена. Без этого @example.com／x,
  /example.com／x читались как домены → PATH; теперь маска ПОСЛЕ целого
  домена с ведущим структурным символом → FREE_TEXT. ВАЖНО (Blocker 1,
  исправление обхода, который D-DET-4 сам создал): ведущий структурный
  символ + домен-С-маской (#goog<mask>le.com, @/:/?/\ аналогично) —
  это HOST, а не FREE_TEXT. Разница: маска ВНУТРИ домена после префикса
  → HOST; маска ПОСЛЕ целого домена → FREE_TEXT. Префикс демотит только
  path-случай, никогда не глушит HOST.
- ИСПРАВЛЕНО с 2026-07-12 (FIX_FIRST, круг 2 → Blocker 2): полноширинные
  и иероглифические доменные точки — U+FF0E (．), U+3002 (。), U+FF61 (｡)
  — канонизируются в ASCII-точку ПЕРЕД разбором меток. Объект защиты
  сам полноширинный (／ = U+FF0F), поэтому атакующий естественно ставит
  и полноширинную точку; без канонизации выборка обрывалась на ней и
  домен пропадал → FREE_TEXT. ASCII-домены не затронуты.
- ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ first-run-only (2026-07-12, круг 2; не чинится):
  детектор проверяет только ПЕРВЫЙ domain-shaped run в токене. Домен
  после ИНОГО (не структурного) разделителя внутри того же токена —
  | = & + — не распознаётся (tag|goog<mask>le.com → FREE_TEXT). НЕ
  обещаем «весь класс внешней пунктуации»: закрыт класс ОБРАМЛЕНИЯ
  домена, не «найти домен в любом месте произвольного токена».
...
- ОТЛОЖЕНО на v0.5 (круг 2, записано — не блокеры для РУЧНОГО
  инструмента: человек вставляет текст и читает вердикт в окне, нет
  downstream-потребителя, которому провалиться в шов):
  (1) SequenceOutput.as_dict() НЕ несёт relation_verdicts — потребитель
  сериализованной формы потерял бы вердикты маски; для окна неважно,
  но зафиксировано;
  (2) TLD-health: набор размером ≥100 считается здоровым, даже если в
  нём отсутствует «com» — усилить позже sentinel-проверкой на наличие
  com/org;
  (3) отношение сигнал/шум режима DEGRADED не измерено.
