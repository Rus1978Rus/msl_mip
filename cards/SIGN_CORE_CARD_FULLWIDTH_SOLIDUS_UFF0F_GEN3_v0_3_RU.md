...
RISK_CASES:
  ...
  RISK_CASE_005:
    ...
    EXAMPLE: a.b／c без http://
    INTERPRETATION: schemeless_domain_masking
    RISK: MEDIUM
    RULE: НЕ ловится текущим грубым детектором (граница Г1, честно
      зафиксирована в LIMITATION_STATEMENT); поверхность признана
...
- Карточка НЕ ловит: голые домены без схемы (a.b／c → FREE_TEXT в
  текущем грубом детекторе), маску в порту (порт считается частью
  HOST — известная граница детектора), EMAIL-контекст (вне scope
  первого захода).
