...
RISK_CASES:
  RISK_CASE_001:
    NAME: PATH_TRAVERSAL_MIMICRY
    INPUT: "../../etc/passwd"
    CONTEXT: солидус участвует в последовательности, похожей на
      directory traversal
    RISK: HIGH
    ATTACK: визуально узнаваемый паттерн обхода файловой системы;
      сам по себе текст не даёт доступа, но может быть частью
      реальной попытки обхода в системах, которые буквально
      резолвят путь
...
PATCHES_APPLIED: 4
PATCHES_VERIFIED: 2/2 (содержательные патчи 01-02; патчи 03-04 —
